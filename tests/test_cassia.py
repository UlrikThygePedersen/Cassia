...
def test_cassia_tidal_windows(
    cassia_instance, load_vessels, load_ports, vessel_imo, unlocode
):
    ...
    tidal_windows, time_range, total_depths = cassia_instance.get_tidal_windows(
        imo=vessel["IMO"], unlocode=unlocode, arrival_time=arrival_time
    )

    assert isinstance(tidal_windows, list), "Tidal windows should be a list."
    assert all(
        isinstance(window, tuple) for window in tidal_windows
    ), "Each window should be a tuple."
    assert all(
        len(window) == 2 for window in tidal_windows
    ), "Each window tuple should contain start and end times."
    assert (
        len(tidal_windows) > 0
    ), f"No tidal windows found for vessel {vessel['NAME']} at port {unlocode}."
...
def test_vessels_with_windows(
    cassia_instance, load_vessels, load_ports, vessel, unlocode
):
...
@pytest.mark.parametrize("unlocode", ports)
def test_vessels_without_windows(
    cassia_instance, load_vessels, load_ports, vessel, unlocode
):
    ...
    if (
        vessel["draught"]
        >= load_ports.loc[load_ports["UNLOCODE"] == unlocode][
            "APPROACH_MLLW_METERS"
        ].values[0]
    ):
        assert (
            len(tidal_windows) == 0
        ), f"Tidal windows should not be available for vessel {vessel['name']} at port {unlocode}."
    else:
        assert (
            len(tidal_windows) > 0
        ), f"Tidal windows should be available for vessel {vessel['name']} at port {unlocode}."
