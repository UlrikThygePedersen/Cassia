...
@given(
    vessel_imo=st.sampled_from([9790933, 9331866, 9251315, 9494008, 9461128, 9473121, 9582116, 9991234, 9758571, 9591765]),
    unlocode=st.sampled_from(['AUBNE', 'AUABP', 'AUDAM', 'AUCTN']),
    draught=st.floats(min_value=5.0, max_value=40.0),
    arrival_time=st.datetimes(min_value=pd.Timestamp('2024-01-01'), max_value=pd.Timestamp('2024-12-31'))
)
def test_tidal_windows_with_hypothesis(cassia_instance, load_ports, load_vessels, vessel_imo, unlocode, draught, arrival_time):
    """Test tidal windows calculation using Hypothesis for property-based testing."""
    
    # Get the MLLW of the port
    port_mllw = load_ports.loc[load_ports['UNLOCODE'] == unlocode]['APPROACH_MLLW_METERS'].values[0]
    
    # Adjust draught in the vessels DataFrame to use Hypothesis-generated draught
    ...
    if draught >= port_mllw:
        assert len(tidal_windows) == 0, f"Unexpected tidal windows for draught {draught} and port {unlocode}."
    else:
        assert len(tidal_windows) > 0, f"No tidal windows found for draught {draught} and port {unlocode}."
    ...
    for index, vessel in vessels_df.iterrows():
        for index, port in ports_df.iterrows():
            ...
            if vessel['DRAUGHT'] >= port['APPROACH_MLLW_METERS']:
                assert len(tidal_windows) == 0, f"Unexpected tidal windows for vessel {vessel['NAME']} and port {port['UNLOCODE']}."
            else:
                assert len(tidal_windows) > 0, f"No tidal windows found for vessel {vessel['NAME']} and port {port['UNLOCODE']}."
